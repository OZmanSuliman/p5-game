...
function setup() {
    ...
    startGame()
    platforms = []
    platforms.push(createPlatforms(300, floorPos_y - 40, 60))
    platforms.push(createPlatforms(500, floorPos_y - 80, 110))
    platforms.push(createPlatforms(700, floorPos_y - 60, 200))
...
function drawGameChar() {
    ...
    if (!isOverCanyon) {
        ...
        if ((floorPos_y != gameChar_y) && !isPlummeting && stepsDown < gravity && !isOverCanyon) {
            ...
            for (let index = 0; index < platforms.length; index++) {
                isContact = 
                if (platforms[index].checkContact(gameChar_world_x, gameChar_y) == true) { break; }
            }
            sleep(200).then(function () {
