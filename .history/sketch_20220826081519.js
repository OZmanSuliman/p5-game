...
function setup() {
    ...
    startGame()
    platforms = []
    platforms.push(createPlatforms(1000, floorPos_y - 40, 100))
    platforms.push(createPlatforms(1500, floorPos_y - 80, 110))
    platforms.push(createPlatforms(750, floorPos_y - 60, 100))
    platforms.push(createPlatforms(1900, floorPos_y - 80, 100))
}
...
function draw() {
    ...
    for (let index = 0; index < enemies.length; index++) {
        ...
        element.draw()
        isEnemyContact = element.checkContact(gameChar_world_x, gameChar_y);
        if (isEnemyContact) {
            break;
        }
    }
...
function keyPressed() {
    if (isGameEnded) {
        ...
        if (keyCode == "39") {
            ...
            isLeft = true;
        } else if (keyCode == "32" && (floorPos_y == gameChar_y || isPlatformContact)) {
            isPlummeting = true;
            jumpSound.play();
        }
    }
...
function drawGameChar() {
    ...
    if (!isOverCanyon) {
        ...
        for (let index = 0; index < platforms.length; index++) {
            ...
            if (platforms[index].checkContact(gameChar_world_x, gameChar_y) == true) {
                isPlatformContact = true
                temporaryPlatformDetails = platforms[index];
                break;
            } else {
        ...
        if (isPlummeting && temporaryPlatformDetails != null && temporaryPlatformDetails.y - gameChar_y < 100 && stepsUp < gravity && !isOverCanyon) {
            // rising
            stepsUp++;
            sleep(200).then(function () {
                gameChar_y -= 100 / gravity;
...
async function checkPlayerDie() {
    if (isOverCanyon || isEnemyContact) {
        fallSound.play();
...
function Enemy(x, y, range) {
    ...
    this.draw = function () {
        ...
        ellipse(this.currentX - 7, this.y + 15, 10, 10);
        ellipse(this.currentX + 7, this.y + 15, 10, 10);
        fill(255)
        ellipse(this.currentX - 7, this.y, 5, 5);
        ellipse(this.currentX + 7, this.y, 5, 5);
    }
